...
int menu() {
	...
	outtextxy(450, 400, "软件八班第一小组原创");


	while (true) {
		m = GetMouseMsg();
		if (m.uMsg == WM_LBUTTONDOWN) {
			if (25 <= m.x&&m.x <= 173&& 35 <= m.y&&m.y <= 115) {
				cleardevice();
				FlushMouseMsgBuffer();
				luruhanshu();//录入函数
				goto kaishi;
...
void luruhanshu() {
	...
	char luru20char[5];//入院时间——月,转int
	char luru21char[5];//入院时间——天，转int
	char luru22char[5];//入院时间——时，转int
	char luru23char[5];//入院时间——分,转int

	char luru24char[3];//flag 1.代表后面输入检查  2.代表后面输入为用药 3.代表其后的诊疗信息为住院
...
addstart:
	...
	while (true) {
		...
		if (m.uMsg == WM_LBUTTONDOWN) {
			...
			zhenliaostart:
				InputBox(luru24char, 20, "诊疗情况（1-检查，2-用药，3-住院)");//
	...
	while (true) {
		...
		if (m.uMsg == WM_LBUTTONDOWN) {
			if (620 <= m.x&&m.x <= 715 && 300 <= m.y&&m.y <= 350) {
				...
				while (shiduanzl3int > 11 || shiduanzl3int < 0)
				{
				...
				while (shiduanzl7int > 11 || shiduanzl7int < 0)
				{
